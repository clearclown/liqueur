...
## Claude Artifacts / Gemini Canvas との関係
...
これらは AI との会話の中で、コード・ドキュメント・ダッシュボードなどの「成果物」をリアルタイムに生成・編集できる画期的な機能です。

**Liquid Protocol は、この概念を「自分のアプリ」に組み込むためのプロトコルです。**
...
|------|-----------------|---------------|---------------------|
| AI による UI 生成 | ✅ | ✅ | ✅ |
| 自分のアプリに組み込み | ❌ Claude 内のみ | ❌ Gemini 内のみ | **✅ どこでも** |
| 自分のデータベース接続 | ❌ | ❌ | **✅** |
| Row-Level Security | ❌ | ❌ | **✅** |
| コード実行のリスク | ⚠️ サンドボックス | ⚠️ サンドボックス | **✅ コード生成なし** |
| オープンソース | ❌ | ❌ | **✅ MIT ライセンス** |
| AI プロバイダー選択 | ❌ Claude のみ | ❌ Gemini のみ | **✅ 任意** |
...
Claude Artifacts や Gemini Canvas は、AI が生成したコードをサンドボックス内で実行します。これは安全ですが、制限もあります。

**Liquid Protocol は根本的に異なるアプローチを取ります：**

```
Claude Artifacts:  AI → JavaScript コード生成 → サンドボックスで実行
Gemini Canvas:     AI → JavaScript コード生成 → サンドボックスで実行
Liquid Protocol:   AI → JSON スキーマのみ → 実行コードなし ✅
```

コードを生成しないので、XSS や任意コード実行のリスクがゼロです。
